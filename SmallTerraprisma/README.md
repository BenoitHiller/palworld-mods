...
So I'm aware of the following quirks and hope to fix them in the future:

* The particle effects are not scaled down to match the model.
* I had to remove the cool after-image style effect on the swords because when not scaled it renders huge bright lines as big as the swords normally are.
* I want to reduce the spacing between the two sets of blades, but that spacing is implemented in code that I couldn't get at easily.
...
3. Make sure the checkboxes next to Smaller Terraprisma, UE4SS Experimental (Palworld), and PalSchema are **CHECKED**
